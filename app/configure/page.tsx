...
export default function ConfigurePage() {
  ...
  const setupTotal = selectedPlan.custom ? null : selectedPlan.price + needs.length * 25
  const monthlyTotal = selectedPlan.custom ? null : selectedPlan.monthlyPrice + needs.length * 10
  ...
  async function saveConfiguration(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/onboarding", {
        ...
        body: JSON.stringify({ companyName, contactEmail, website, planId, needs, setupTotal, monthlyTotal }),
      })
